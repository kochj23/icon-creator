...
class NovaAPIServer {
    ...
    func start() {
        do {
            ...
            listener?.stateUpdateHandler = { if case .ready = $0 { print("NovaAPI [IconCreator]: port \(self.port)") } }
            listener?.start(queue: .main)
        } catch { print("NovaAPI [IconCreator]: failed — \(error)") }
    }
